...
def _load_page_modules() -> List[ModuleType]:
    ...
    for filepath in sorted(pages_dir.glob("*.py")):
        if filepath.stem in EXCLUDED_STEMS:
            continue
        modules.append(importlib.import_module(f"pages.{filepath.stem}"))
...
def _module_to_page_info(mod: ModuleType) -> PageInfo:
    ...
    return PageInfo(
        ...
        title=title,
        url_path=url_path,
        page=st.Page(build_page, title=title, url_path=url_path),
    )


def _page_infos() -> List[PageInfo]:
    return [_module_to_page_info(mod) for mod in _load_page_modules()]


def load_page_sections() -> Dict[str, List[st.Page]]:
...
def load_page_urls() -> Dict[str, st.Page]:
    return {info.url_path: info.page for info in _page_infos()}
